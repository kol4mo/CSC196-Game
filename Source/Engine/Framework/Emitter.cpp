...
void hop::Emitter::Update(float dt)
{
	Actor::Update(dt);
...
void hop::Emitter::Draw(hop::Renderer& renderer)
{
	//
}

void hop::Emitter::Emit()
{
	Particle* particle = hop::g_particleSystem.GetFreeParticle();
	if (particle)
	{
		ParticleData data;
